import math

# Glicko-2 constants
TAU = 0.5  # System constant, usually between 0.3 and 1.2
MU = 1500.0
SCALE = 173.7178

# Helper functions for Glicko-2


def g(phi):
    return 1 / math.sqrt(1 + 3 * (phi**2) / (math.pi**2))


def E(mu, mu_j, phi_j):
    return 1 / (1 + math.exp(-g(phi_j) * (mu - mu_j)))


def compute_variance(mu, opponents):
    v_inv = 0
    for mu_j, phi_j, s_j in opponents:
        E_j = E(mu, mu_j, phi_j)
        v_inv += (g(phi_j) ** 2) * E_j * (1 - E_j)
    return 1 / v_inv


def compute_delta(mu, phi, v, opponents):
    delta_sum = 0
    for mu_j, phi_j, s_j in opponents:
        E_j = E(mu, mu_j, phi_j)
        delta_sum += g(phi_j) * (s_j - E_j)
    return v * delta_sum


def update_volatility(phi, delta, v, sigma):
    a = math.log(sigma**2)
    tau = TAU
    epsilon = 1e-6

    def f(x):
        exp_x = math.exp(x)
        num = exp_x * (delta**2 - phi**2 - v - exp_x)
        den = 2 * (phi**2 + v + exp_x) ** 2
        return num / den - (x - a) / (tau**2)

    A = a
    B = None
    if delta**2 > phi**2 + v:
        B = math.log(delta**2 - phi**2 - v)
    else:
        B = a - 1.0

    fA = f(A)
    fB = f(B)

    while abs(B - A) > epsilon:
        C = A + (A - B) * fA / (fB - fA)
        fC = f(C)

        if fC * fB < 0:
            A = B
            fA = fB
        else:
            fA /= 2

        B = C
        fB = fC

    return math.exp(A / 2)


def f(x, delta, phi, v, a):
    exp_x = math.exp(x)
    num = exp_x * (delta**2 - phi**2 - v - exp_x)
    den = 2 * (phi**2 + v + exp_x) ** 2
    return num / den - (x - a) / (TAU**2)


def glicko2_rating_update(player_rating, game_results):
    mu = (player_rating["rating"] - MU) / SCALE
    phi = player_rating["rd"] / SCALE
    sigma = player_rating["volatility"]

    if not game_results:
        phi_star = math.sqrt(phi**2 + sigma**2)
        return {"rating": MU + SCALE * mu, "rd": SCALE * phi_star, "volatility": sigma}

    opponents = []
    for result in game_results:
        opp_mu = (result["opponent_rating"] - MU) / SCALE
        opp_phi = result["opponent_rd"] / SCALE
        s = result["score"]
        opponents.append((opp_mu, opp_phi, s))

    v = compute_variance(mu, opponents)
    delta = compute_delta(mu, phi, v, opponents)
    sigma_prime = update_volatility(phi, delta, v, sigma)
    phi_star = math.sqrt(phi**2 + sigma_prime**2)
    phi_prime = 1 / math.sqrt(1 / (phi_star**2) + 1 / v)

    sum_term = 0
    for mu_j, phi_j, s_j in opponents:
        E_j = E(mu, mu_j, phi_j)
        sum_term += g(phi_j) * (s_j - E_j)

    mu_prime = mu + phi_prime**2 * sum_term

    return {
        "rating": MU + SCALE * mu_prime,
        "rd": SCALE * phi_prime,
        "volatility": sigma_prime,
    }


def process_rating_period(previous_ratings, game_results):
    updated_ratings = {}
    for player, player_data in previous_ratings.items():
        player_games = [gr for gr in game_results if gr["player"] == player]
        updated = glicko2_rating_update(player_data, player_games)
        updated_ratings[player] = updated
    return updated_ratings


# Example Usage
if __name__ == "__main__":
    previous_ratings = {
        "playerA": {"rating": 1500, "rd": 100, "volatility": 0.06},
        "playerB": {"rating": 1500, "rd": 100, "volatility": 0.06},
        "playerC": {"rating": 1500, "rd": 100, "volatility": 0.06},
    }

    game_results = [
        {"player": "playerA", "opponent_rating": 1400, "opponent_rd": 30, "score": 1},
        {"player": "playerA", "opponent_rating": 1550, "opponent_rd": 100, "score": 0},
        {"player": "playerB", "opponent_rating": 1500, "opponent_rd": 200, "score": 0},
        {"player": "playerC", "opponent_rating": 1500, "opponent_rd": 200, "score": 1},
    ]

    updated_ratings = process_rating_period(previous_ratings, game_results)
    for player, data in updated_ratings.items():
        print(player, data)
